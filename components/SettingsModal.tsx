...
const SettingsModal: React.FC<SettingsModalProps> = ({ currentMapUrl, currentPublicUrl, onSave, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/95 backdrop-blur-lg p-4">
      <div className="bg-gray-900 w-full max-w-4xl rounded-[2.5rem] border border-gray-800 shadow-2xl flex flex-col overflow-hidden animate-in zoom-in duration-300">
        
        {/* Header */}
        <div className="flex justify-between items-center p-8 bg-black/40 border-b border-gray-800">
          <div>
            ...
            <p className="text-gray-500 text-sm mt-1">Konfigurera din industriella SOP-portal</p>
          </div>
        ...
        <div className="p-8 space-y-10 overflow-y-auto max-h-[70vh]">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
              ...
              {!isLocalhost ? (
                ...
                <div className="space-y-4">
                  ...
                  <div className="relative">
                    <Link className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-600" size={20} />
                    <input 
                      type="text" 
                      value={publicUrl}
                      onChange={(e) => setPublicUrl(e.target.value)}
                      placeholder="t.ex. 192.168.1.50"
                      className="w-full bg-black border-2 border-gray-800 rounded-2xl px-6 pl-14 py-5 text-white focus:border-blue-600 outline-none font-mono text-lg transition-all"
                    />
                  </div>
                  <div className="bg-amber-950/20 border border-amber-900/30 p-4 rounded-xl text-[11px] text-amber-400 leading-relaxed">
            ...
            {/* LIVE PREVIEW BOX */}
            <div className="bg-black/40 rounded-[2rem] p-8 border border-gray-800 flex flex-col items-center justify-center text-center space-y-6">
              <div className="space-y-2 w-full">
                <span className="text-[10px] font-black text-gray-600 uppercase tracking-widest">Aktiv länk för QR-koder</span>
                <div className="bg-gray-950 p-4 rounded-xl border border-gray-800 flex items-center justify-between">
                  ...
                  <button onClick={handleCopyLink} className="p-2 hover:bg-gray-800 rounded-lg text-gray-500 hover:text-white">
          ...
          <div className="pt-8 border-t border-gray-800 space-y-4">
            <h3 className="text-lg font-black text-gray-400 uppercase tracking-widest flex items-center gap-2 italic">
              <ImageIcon size={20} /> Maskinritning (Layout)
            </h3>
            <div className="flex gap-8 items-center bg-black/40 p-8 rounded-[2rem] border border-gray-800">
               <div className="w-56 aspect-video bg-gray-950 rounded-2xl overflow-hidden border border-gray-800 flex items-center justify-center">
               ...
               <div className="flex-1 space-y-4">
                 ...
                 <button onClick={() => fileInputRef.current?.click()} className="w-full py-4 bg-gray-800 hover:bg-gray-700 text-white rounded-2xl font-bold flex items-center justify-center gap-3 border border-gray-700 transition-all">
        ...
        {/* Footer */}
        <div className="p-8 bg-black/40 border-t border-gray-800 flex justify-end gap-4">
          <button onClick={onClose} className="px-8 py-4 text-gray-500 hover:text-white font-bold transition-colors">Avbryt</button>
